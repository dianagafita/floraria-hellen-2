...
//   console.log("AAAAAA", formState);

//   useEffect(() => {
//     console.log("BBBBBBBAAAAA", formState.status);

//     if (formState.status === "success") {
//       window.location.href = window.location.origin; // Reloads the entire site.
//     }
//   }, [formState.status]);
...
export default function AuthForm({ mode }) {
  const [formState, setFormState] = useState({ status: "", errors: {} });
  const router = useRouter();

  async function handleSubmit(event) {
    event.preventDefault();
    setFormState({ status: "pending", errors: {} });

    const formData = new FormData(event.target);
    const response = await auth(mode, null, formData);

    if (response.errors) {
      setFormState({ status: "error", errors: response.errors });
    } else {
  ...
  const switchMode = () => {
    ...
    router.push(`/authentification/?mode=${newMode}`);
  };
    ...
    <>
      ...
      <div className="flex flex-col my-10 py-10 mb-40 mx-5 md:mx-20  items-center rounded-sm  bg-white  shadow-lg border border-[0.1px] border-opacity-5 border-black">
        ...
        >
          ...
          <p
            className="text-[13px] font-[100] underline underline-offset-2 text-[#707070] hover:text-[#404040] cursor-pointer"
            onClick={switchMode}
          >
            {mode === "login"
              ? "Nu ai un cont? Creeaza unul acum."
